...
pub trait FloatType: Copy + Sized {
...
impl Tensor {
    ...
    */
    ...
    /*
      For [-1, 1], data is "Zero-Centered" (the range is centered at 0.0 and values are
      bi-polar, meaning they are both positive and negative):
      $$pixel_{float} = \left(\frac{pixel_{u8}}{127.5}\right) - 1.0$$

      For [0, 1], data is in the "Standard Unit Range" (values are uni-polar, meaning
      all values are non-negative. This is a Min-Max scaling that transforms values
      into a proportional unit of the total possible range):
      $$pixel_{float} = \frac{pixel_{u8}}{255.0}$$
